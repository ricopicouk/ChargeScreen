...
static constexpr uint16_t MIN_SOLAR_ARRAY_WATTS = 50;
static constexpr uint16_t MAX_SOLAR_ARRAY_WATTS = 2000;
static constexpr uint16_t SOLAR_ARRAY_WATTS_STEP = 5;
static constexpr uint16_t DEFAULT_SCREEN_TIMEOUT_SECONDS = 30;
static constexpr uint16_t MIN_SCREEN_TIMEOUT_SECONDS = 10;
static constexpr uint16_t MAX_SCREEN_TIMEOUT_SECONDS = 300;
static constexpr uint16_t SCREEN_TIMEOUT_STEP_SECONDS = 10;
...
static constexpr int16_t SETTINGS_NEXT_X = 68;
static constexpr int16_t SETTINGS_NEXT_Y = 204;
static constexpr int16_t SETTINGS_NEXT_W = 104;
static constexpr int16_t SETTINGS_NEXT_H = 26;
...
static constexpr int16_t SETTINGS_SCREEN_TIMEOUT_Y = 76;
static constexpr int16_t SETTINGS_DIMMING_Y = 108;
static constexpr int16_t SETTINGS_BATTERY_Y = 140;
static constexpr int16_t SETTINGS_LABELS_Y = 172;
...
static bool settingsInstructionsActive = false;
static bool settingsInstructionsShownThisBoot = false;
...
static bool hexToBuffer(String hex, uint8_t *out, size_t maxLen, size_t &outLen) {
  hex.trim();
  hex.replace(" ", "");
  hex.replace(":", "");
  hex.replace("-", "");
  ...
  if (hex.length() == 0 || (hex.length() % 2) != 0 || hex.length() / 2 > maxLen) {
    return false;
  }
...
static uint16_t sanitiseScreenTimeoutSeconds(int value) {
  if (value < MIN_SCREEN_TIMEOUT_SECONDS) {
...
static void loadStoredSecrets() {
  ...
  if (solarArrayWatts < MIN_SOLAR_ARRAY_WATTS || solarArrayWatts > MAX_SOLAR_ARRAY_WATTS) {
    solarArrayWatts = DEFAULT_SOLAR_ARRAY_WATTS;
  ...
  if (screenTimeoutSeconds < MIN_SCREEN_TIMEOUT_SECONDS ||
      screenTimeoutSeconds > MAX_SCREEN_TIMEOUT_SECONDS) {
    screenTimeoutSeconds = DEFAULT_SCREEN_TIMEOUT_SECONDS;
...
static bool readTouchPoint(TouchPoint &point) {
  ...
  if (point.x < 0 || point.x > 239 || point.y < 0 || point.y > 239) {
    point.touched = false;
  }
  return true;
}
...
static void drawSettingsNextButton() {
  gfx->drawRect(SETTINGS_NEXT_X, SETTINGS_NEXT_Y, SETTINGS_NEXT_W, SETTINGS_NEXT_H, 0x6B4D);
  gfx->fillRect(SETTINGS_NEXT_X + 1, SETTINGS_NEXT_Y + 1, SETTINGS_NEXT_W - 2, SETTINGS_NEXT_H - 2, 0x01EB);
  drawAaCentered(AA_FONT_SMALL, "Settings", 120, SETTINGS_NEXT_Y - 5, WHITE);
}
...
static String settingsStatusMessage(const String &code) {
  ...
  if (code == "invalid") {
    return "Check key/password/wattage/timeout format.";
  }
  if (code == "ota-ok") {
...
static String settingsPageHtml(const String &message = "") {
  ...
  html += F("<h1>ChargeScreen Settings</h1>");
  ...
  html += F("<p class='hint'>Set solar array watts to match the size of your solar panel and paste Victron Instant Readout decryption keys. Connect your phone to the ChargeScreen WiFi hotspot, then open 192.168.4.1.</p>");
  html += F("<p class='hint'>Victron key help: open VictronConnect, select the SmartShunt or solar controller, open Product info or Settings, enable Instant Readout if needed, then copy the Instant Readout encryption key. This is different from the Bluetooth pairing PIN printed on the device.</p>");
  html += F("<form method='post' action='/save' data-working='Saving...'>");
  html += F("<label for='solar_watts'>Solar array watts</label><input id='solar_watts' name='solar_watts' type='number' min='");
  ...
  html += F("'>");
  html += F("<label for='screen_timeout'>Screen off delay (seconds)</label><input id='screen_timeout' name='screen_timeout' type='number' min='");
  html += MIN_SCREEN_TIMEOUT_SECONDS;
  html += F("' max='");
  html += MAX_SCREEN_TIMEOUT_SECONDS;
  html += F("' step='");
  html += SCREEN_TIMEOUT_STEP_SECONDS;
  html += F("' value='");
  html += screenTimeoutSeconds;
  html += F("'>");
...
static void handleSettingsSave() {
  ...
  if (screenTimeoutText.length()) {
    ...
    if (requestedSeconds >= MIN_SCREEN_TIMEOUT_SECONDS &&
        requestedSeconds <= MAX_SCREEN_TIMEOUT_SECONDS) {
...
static void handleFirmwareUpload() {
  ...
  if (upload.status == UPLOAD_FILE_START) {
    ...
    if (firmwareUploadSize > UPDATE_SIZE_UNKNOWN) {
      firmwareUploadFailed = true;
      Serial.println("OTA rejected: oversized upload");
    }
...
static void drawSettingsInstructionsPage(bool force = false) {
  ...
  drawAaCentered(AA_FONT_SMALL, "Setup", 120, 28, WHITE);

  gfx->setFont();
  gfx->setTextSize(1);
  gfx->setTextColor(COLOR_DIM_TEXT);
  gfx->setCursor(38, 56);
  gfx->print("Switch on WiFi.");
  gfx->setCursor(38, 74);
  gfx->print("Use phone to add");
  gfx->setCursor(38, 90);
  gfx->print("decryption codes.");
  gfx->setCursor(38, 114);
  gfx->print("Config page:");
  gfx->setTextColor(WHITE);
  gfx->setCursor(38, 130);
  gfx->print("192.168.4.1");
  gfx->setTextColor(COLOR_DIM_TEXT);
  gfx->setCursor(38, 154);
  gfx->print("Adjust solar to");
  gfx->setCursor(38, 170);
  gfx->print("solar panel size.");

  drawSettingsNextButton();
...
static void drawSettingsPage(bool force = false) {
  ...
  drawAaCentered(AA_FONT_SMALL, "Settings", 120, 34, WHITE);
  ...
  if (settingsPageIndex == 0) {
    ...
    drawStepperRow(SETTINGS_SCREEN_TIMEOUT_Y, "Timeout", String(screenTimeoutSeconds) + "s");
...
static void drawSettingsInfoPage(bool force = false) {
  ...
  drawInfoLine(184, "Screen", String(screenTimeoutSeconds) + "s");
...
static void drawSolarFace() {
  gfx->fillScreen(COLOR_BLACK_SOFT);
  gfx->fillCircle(120, 120, 120, COLOR_SOLAR_PANEL);
  gfx->drawCircle(120, 120, 119, COLOR_SOLAR_PANEL);
  drawSegmentedGaugeArcAt(120, 0.0f, COLOR_SOLAR_TRACK, COLOR_SOLAR_ORANGE, COLOR_SOLAR_PANEL);
  gfx->fillRect(96, 199, 48, 25, COLOR_SOLAR_PANEL);
  gfx->fillCircle(120, 120, GAUGE_CLEAR_RADIUS, COLOR_SOLAR_PANEL);
  drawSettingsButton(COLOR_SOLAR_PANEL, COLOR_SOLAR_TEXT);
  pageTwoDrawn = true;
  solarValuesDrawn = false;
}

static void drawSolarValues(const SolarStats &stats,
                            const String &powerText,
                            const String &statusText,
                            const String &batteryText,
                            const String &currentText,
                            const String &yieldText,
                            const String &errorText,
                            bool force) {
  if (force || !solarValuesDrawn || powerText != lastSolarPowerText) {
    float maxPower = max(1.0f, static_cast<float>(solarArrayWatts));
    float ringPower = stats.valid && !isnan(stats.pvPower)
                          ? constrain(stats.pvPower, 0.0f, maxPower)
                          : 0.0f;
    drawSegmentedGaugeArcAt(120,
                            GAUGE_ARC_SWEEP_DEG * ringPower / maxPower,
                            COLOR_SOLAR_TRACK,
                            COLOR_SOLAR_ORANGE,
                            COLOR_SOLAR_PANEL);
  ...
  if (stats.valid) {
    ...
    drawAaCentered(AA_FONT_LARGE, powerText, 120, 43, COLOR_SOLAR_TEXT);

    drawAaCentered(AA_FONT_SMALL, batteryText, 78, 112, COLOR_SOLAR_TEXT);
    drawAaCentered(AA_FONT_SMALL, currentText, 164, 112, COLOR_SOLAR_TEXT);
    if (showValueLabels) {
      drawTinyLabel("BATTERY", 78, 139, COLOR_SOLAR_DIM_TEXT);
      drawTinyLabel("CHARGE", 164, 139, COLOR_SOLAR_DIM_TEXT);
    }

    drawAaCentered(AA_FONT_SMALL, yieldText, 120, 148, COLOR_SOLAR_TEXT);
    if (showValueLabels) {
      drawTinyLabel("TODAY", 120, 175, COLOR_SOLAR_DIM_TEXT);
    }
    drawAaCentered(AA_FONT_SMALL, statusText, 120, 184, COLOR_SOLAR_DIM_TEXT);
  } else {
    drawAaCentered(AA_FONT_LARGE, "-- W", 120, 50, COLOR_SOLAR_TEXT);
    drawAaCentered(AA_FONT_SMALL, statusText, 120, 106, COLOR_SOLAR_DIM_TEXT);
...
static void drawPageTwo(bool force = false) {
  ...
  drawSolarValues(stats, powerText, statusText, batteryText, currentText, yieldText, errorText, force);
...
static void drawCurrentPage(bool force) {
  ...
  if (settingsInstructionsActive) {
    drawSettingsInstructionsPage(force);
    return;
  }
...
static void switchPage(uint8_t page, int8_t direction = 1) {
  if (settingsInstructionsActive || settingsPageActive || settingsRotationPageActive || settingsInfoPageActive) {
    return;
  }
...
static void togglePageFromSwipe(int8_t direction) {
  switchPage(currentPage == 0 ? 1 : 0, direction);
}

static void openSettingsPage() {
  settingsPageIndex = 0;
  settingsInstructionsActive = !settingsInstructionsShownThisBoot;
  settingsPageActive = settingsInstructionsShownThisBoot;
  settingsInstructionsShownThisBoot = true;
...
static void closeSettingsPage() {
  settingsInstructionsActive = false;
...
static void adjustScreenTimeoutSeconds(int deltaSeconds) {
  uint16_t nextSeconds = sanitiseScreenTimeoutSeconds(static_cast<int>(screenTimeoutSeconds) + deltaSeconds);
  if (nextSeconds == screenTimeoutSeconds) {
    return;
  }
  ...
  screenTimeoutSeconds = nextSeconds;
  secrets.putUShort("screen_timeout", screenTimeoutSeconds);
  settingsMessage = String("Timeout ") + String(screenTimeoutSeconds) + "s";
  settingsPageDrawn = false;
  noteTouchActivity();
  drawCurrentPage(true);
}
...
static void handleTap(int16_t x, int16_t y) {
  ...
  if (settingsInstructionsActive) {
    if (pointInRect(x, y, SETTINGS_NEXT_X, SETTINGS_NEXT_Y, SETTINGS_NEXT_W, SETTINGS_NEXT_H)) {
      settingsInstructionsActive = false;
      settingsPageActive = true;
      settingsPageIndex = 0;
      settingsPageDrawn = false;
  ...
  if (settingsPageActive) {
    if (settingsPageIndex == 1) {
      if (pointInRect(x, y, 138, SETTINGS_SCREEN_TIMEOUT_Y, 24, SETTINGS_ROW_H)) {
        adjustScreenTimeoutSeconds(-SCREEN_TIMEOUT_STEP_SECONDS);
        return;
      }

      if (pointInRect(x, y, 202, SETTINGS_SCREEN_TIMEOUT_Y, 24, SETTINGS_ROW_H)) {
        adjustScreenTimeoutSeconds(SCREEN_TIMEOUT_STEP_SECONDS);
        return;
      }
...
static void updateTouchSwipe() {
  ...
  if ((settingsInstructionsActive || settingsPageActive) &&
      durationMs < 1000 &&
...
void loop() {
  ...
  if (screenAwake && millis() - lastDraw > 1000) {
    lastDraw = millis();
    drawCurrentPage();
  }
