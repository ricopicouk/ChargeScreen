...
static constexpr const char *FIRMWARE_VERSION = "beta2.02";
...
static void drawSettingsInfoPage(bool force = false) {
  ...
  drawInfoLine(144, "Shunt key", savedStatus(storedVictronKey.length() > 0));
  drawInfoLine(164, "Solar key", savedStatus(storedSolarKey.length() > 0));
  drawInfoLine(184, "Eco key", savedStatus(storedEcoWorthyPassword.length() > 0));
