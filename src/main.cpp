...
#ifndef VICTRON_SOLAR_INSTANT_READOUT_KEY_HEX
#define VICTRON_SOLAR_INSTANT_READOUT_KEY_HEX VICTRON_INSTANT_READOUT_KEY_HEX
#endif

#ifndef VICTRON_SOLAR_BLE_ADDRESS
#define VICTRON_SOLAR_BLE_ADDRESS ""
#endif

#ifndef VICTRON_SOLAR_MAX_POWER_W
#define VICTRON_SOLAR_MAX_POWER_W 500.0f
#endif

#ifndef SETTINGS_AP_SSID
#define SETTINGS_AP_SSID "ChargeScreen"
#endif

#ifndef SETTINGS_AP_PASSWORD
...
static constexpr int16_t SETTINGS_BUTTON_Y = 214;
...
static bool matchesConfiguredSolarAddress(const String &address) {
  String wanted = VICTRON_SOLAR_BLE_ADDRESS;
  if (wanted.length() == 0) {
    return true;
  }
  return normaliseAddress(address) == normaliseAddress(wanted);
...
static const char *activeVictronKey() {
  return storedVictronKey.length() ? storedVictronKey.c_str() : VICTRON_INSTANT_READOUT_KEY_HEX;
}
...
static const char *activeSolarKey() {
  ...
  if (strlen(VICTRON_SOLAR_INSTANT_READOUT_KEY_HEX) > 0) {
    return VICTRON_SOLAR_INSTANT_READOUT_KEY_HEX;
  }
  return activeVictronKey();
}
...
static String settingsPageHtml(const String &message = "") {
  ...
  html += F("label.check{display:flex;gap:10px;align-items:center;margin-top:20px;color:#eef}input[type=checkbox]{width:auto}");
  ...
  html += F(".msg{padding:12px;background:#123345;margin:12px 0}.hint{color:#b8cad2;font-size:14px;line-height:1.45}</style></head><body><main>");
  ...
  html += F("<p class='hint'>Paste a 32 character Instant Readout decrypt key. Saved keys are stored on this device and used after the next BLE advert is received.</p>");
...
static void handleSettingsRoot() {
  ...
  settingsServer.send(200, "text/html", settingsPageHtml());
}
...
static void handleSettingsSave() {
  ...
  if (shuntKey.length()) {
    if (isValidKeyHex(shuntKey)) {
      ...
      message += F("Battery monitor key was not valid. ");
    }
  }
  ...
  if (solarKey.length()) {
    if (isValidKeyHex(solarKey)) {
      ...
      message += F("Solar controller key was not valid. ");
    }
  }
  ...
  settingsServer.send(200, "text/html", settingsPageHtml(message));
...
static void drawSettingsPage(bool force = false) {
  ...
  drawAaCentered(AA_FONT_SMALL, "Settings", 120, 38, WHITE);
...
static void drawPageTwoAtOffset(int16_t offsetX) {
  ...
  if (stats.valid) {
    ...
    drawAaCentered(AA_FONT_SMALL, statusText, centerX, 95, COLOR_SOLAR_DIM_TEXT);
    drawAaCentered(AA_FONT_SMALL, formatOneDecimalOrDash(stats.batteryVoltage, "V"), centerX - 42, 124, COLOR_SOLAR_TEXT);
    drawAaCentered(AA_FONT_SMALL, formatOneDecimalOrDash(stats.batteryCurrent, "A"), centerX + 44, 124, COLOR_SOLAR_TEXT);
    drawAaCentered(AA_FONT_SMALL, formatTwoDecimalOrDash(stats.yieldTodayKwh, "kWh"), centerX - 42, 153, COLOR_SOLAR_TEXT);
    drawAaCentered(AA_FONT_SMALL, formatOneDecimalOrDash(stats.loadCurrent, "A"), centerX + 44, 153, COLOR_SOLAR_TEXT);
  } else {
...
static void drawDashboardAtOffset(int16_t offsetX) {
  ...
  if (stats.valid) {
    drawAaCentered(AA_FONT_LARGE, String(socDisplay) + "%", centerX, 43, WHITE);
    drawAaCentered(AA_FONT_SMALL, statusText, centerX, 95, COLOR_DIM_TEXT);
    drawAaCentered(AA_FONT_SMALL, formatSignedOneDecimal(stats.voltage, "V"), centerX - 42, 124, WHITE);
    drawAaCentered(AA_FONT_SMALL, formatSignedOneDecimal(stats.current, "A"), centerX + 44, 124, WHITE);
    drawAaCentered(AA_FONT_SMALL, formatSignedOneDecimal(stats.power, "W"), centerX - 42, 153, WHITE);
    drawAaCentered(AA_FONT_SMALL, auxDisplayText(), centerX + 44, 153, WHITE);
  } else {
...
static void drawPageTwo(bool force = false) {
  ...
  String loadText = stats.valid ? formatOneDecimalOrDash(stats.loadCurrent, "A") : "";
  String errorText = stats.valid ? "" : (solarStats.error.length() ? solarStats.error : "no solar BLE");
...
static void switchPage(uint8_t page, int8_t direction = 1) {
  if (settingsPageActive || settingsRotationPageActive) {
    return;
  }
